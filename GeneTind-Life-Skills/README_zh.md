# GeneTind Life Skills — GeneTind AI 生命科学技能库

<div align="center">

**最大的开源医疗 AI 技能库，专为 GeneTind 框架设计。**

_1000+ 个精选技能 · 临床医学 · 基因组学 · 药物发现 · 生物信息学 · 医疗器械_
...
**GeneTind Life Skills** 是一个包含 **1000+ 个 AI Agent 技能**的精选集合，覆盖生物医学与临床研究的完整领域。这些技能专为 [OpenClaw](https://github.com/openclaw/openclaw) / [NanoClaw](https://github.com/qwibitai/nanoclaw) —— 基于 Claude 的个人 AI 助手框架 —— 设计，能将通用 AI 智能体转变为强大的医学与科研研究伙伴。
...
| 没有技能             | 配备 GeneTind Life Skills 后               |
| -------------------- | ------------------------------------------ |
...
GeneTind 从以下两个位置加载技能：

| 优先级 | 路径                  | 作用域                 |
| ------ | --------------------- | ---------------------- |
| 高     | `<workspace>/skills/` | 每个工作区独立（推荐） |
| 低     | `~/.genetind/skills/` | 全局，所有 Agent 共享  |

#### 方法一 — 克隆并复制（推荐）

```bash
# 克隆本仓库
git clone https://github.com/lilinji/GeneTind-Life-Skills.git

# 安装到当前工作区的 skills 目录
cp -r GeneTind-Life-Skills/skills/* <your-workspace>/skills/

# 或安装到全局（所有 Agent 均可使用）
cp -r GeneTind-Life-Skills/skills/* ~/.genetind/skills/
```
...
将本仓库的克隆路径永久配置到 `~/.genetind/genetind.json` 中：

```json
{
  "skills": {
    "load": {
      "extraDirs": ["/path/to/GeneTind-Life-Skills/skills"]
    }
  }
}
```
...
for skill in "${SKILLS[@]}"; do
  cp -r GeneTind-Life-Skills/skills/$skill ~/.genetind/skills/
done
```
...
git clone https://github.com/lilinji/GeneTind-Life-Skills.git
cp -r GeneTind-Life-Skills/skills/* /path/to/NanoClaw/container/skills/
